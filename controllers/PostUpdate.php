...
if ($_POST['call'] == "let") {
    ...
    if ($edition_limit >= 5) {
        ...
        alertMessage(Alerts::updateTitle(),Alerts::updateLimit(),false,false);
    } else {
        ...
        if (!empty($_SESSION['UsrPkg'])) {
            ...
            */
            ...
            if (($Entry_Original_Title_Hash == $Entry_Edited_Title_Hash) && ($Entry_Original_Content_Hash == $Entry_Edited_Content_Hash)) {
                ...
                notification(true,Alerts::noupdateNotification());
            } else {
                ...
                if ($r && $vc) {
                    ...
                    //Notification
                    notification(true,Alerts::updateNotification());
                    ...
                    //There is an error
                    echo "<script>$('#form_editPost').dialog('close');</script>";
                    alertMessage(Alerts::updateTitle(),Alerts::errorUpdate(),"reload",false);
                }
            }
        } else {
            //Session is broken
            echo "<script>$('#form_editPost').dialog('close');</script>";
            alertMessage(Alerts::updateTitle(),Alerts::sessionBroke_message(),"transport","index.php");
        }
    } else {
        //Procedure parameter wasn't recognized
        echo "<script>$('#form_editPost').dialog('close');</script>";
        alertMessage(Alerts::updateTitle(),Alerts::parameterError(),"reload",false);
    }
}
